...
use ::game::mugen::format::sff::{self, SffData};
...
impl Fight {
    ...
    pub fn load(&mut self, window: &mut Window) {
        ...
        for player in self.players.iter_mut() {
            let chara_data = &self.characters[player.character_id];
            player.animator = Some(air::Animator::new(chara_data.animations[player.current_animation].clone()));
            player.small_face = sprite_atlas_builder.add_surface(chara_data.sff_data.render_sprite(9000, 0, 0).unwrap());
            player.big_face = sprite_atlas_builder.add_surface(chara_data.sff_data.render_sprite(9000, 1, 0).unwrap());
            {
                ...
                for ref step in animator.animation().steps() {
                    for ref frame in step.frames() {
                        ...
                        if player.image_keys.get(&key) == None {
                            if let Some(rendered_sprite) = chara_data.sff_data.render_sprite(key.group, key.image, 0) {
